...
import { getMyFlashcards } from "@/src/services/user.service";
import { FlashcardApiItem } from "@/src/types/user-api";
...
export default function CardsScreen() {
  const { auth } = useAuth();
  const [cards, setCards] = useState<FlashcardApiItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadCards() {
      ...
      try {
        setLoading(true);
        const response = await getMyFlashcards(auth.accessToken);
        setCards(response.data ?? []);
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "Khong the tai flashcard.";
      ...
      <View style={styles.titleRow}>
        ...
        <View style={styles.progressBlock}>
          <Text style={styles.progressValue}>
            {cards.length > 0 ? `${cardIndex + 1}/${cards.length}` : "0/0"}
          </Text>
          <ProgressBar
            accentColor="#2EB84B"
            value={cards.length > 0 ? ((cardIndex + 1) / cards.length) * 100 : 0}
          />
        </View>
      </View>
      ...
      <Pressable onPress={() => setShowMeaning((current) => !current)}>
        <SurfaceCard style={styles.flashcard}>
          ...
          <Text style={styles.word}>
            {loading ? "Dang tai..." : currentCard?.englishWord ?? "Chua co flashcard"}
          </Text>

          <View style={styles.pronunciationRow}>
            <Text style={styles.pronunciation}>
              {currentCard?.pronunciation ?? "Khong co phien am"}
            </Text>
            <Pressable
              onPress={() =>
                Alert.alert("Audio", `Phat am tu: ${currentCard?.englishWord ?? ""}`)
              }
            ...
            {showMeaning ? (
              <>
                ...
                <Text style={styles.meaningText}>
                  {currentCard?.meaningVi ?? "Khong co nghia"}
                </Text>
                <Text style={styles.exampleLabel}>Example</Text>
                <Text style={styles.exampleText}>
                  {currentCard?.exampleSentence ?? "Khong co vi du"}
                </Text>
              </>
            ) : (
